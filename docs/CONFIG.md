...
| 映射成 `ctrl+alt+cmd+up` 这类带方向键的组合，Raycast 等热键软件不响应，换成字母就好 | 已知问题，待修：macOS 上合成的方向键、Home/End/PageUp/PageDown、F 键少了真实键盘会附加的 fn / 小键盘标志位，按标志位比对的热键软件认不出来。临时办法：热键用字母或数字 |
| 键按了，「最近触发」也有显示，但 Raycast 等快捷键工具不响应；菜单里出现「⚠️ 安全输入被 … 占用」 | macOS 的「安全输入」（Secure Input）被某个应用占着没关。它本是光标在密码框里时防键盘记录的机制，正常离开密码框就自动关闭，但有些应用忘了关。占用期间 MicroKeys 照常发键，靠监听键盘的快捷键工具却收不到。菜单里的那一行会写明占用者（应用名和 PID），点击有处理步骤：切到该应用按 Esc、退出重开该应用、或锁屏再解锁。终端里可用 `ioreg -l -d 1 -w 0 \| grep SecureInput` 确认，`kCGSSessionSecureInputPID` 就是占用者 |
...
| 重新编译后权限又要重新给 | 用了临时签名（ad-hoc）。用固定证书签名（Developer ID 或自签名，见 docs/SIGNING.md），权限就能跨版本保留 |
