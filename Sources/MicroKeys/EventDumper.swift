...
enum EventDumper {
    static func run(seconds: Int) -> Int32 {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            FileHandle.standardError.write(Data("无法创建事件监听：需要给运行它的终端「输入监控」权限。\n".utf8))
            return 1
        }
        let source = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, tap, 0)
        CFRunLoopAddSource(CFRunLoopGetCurrent(), source, .commonModes)
        CGEvent.tapEnable(tap: tap, enable: true)
        print("监听 \(seconds) 秒。先在真实键盘上按一次快捷键，再按一次 Codex Micro 上映射的键，对比两组输出。")
        print("真实按键 srcPid=0；如果合成按键的 srcPid 不是 0 或 kbdType 是 0，就是被目标应用当成合成输入忽略了。")
        fflush(stdout)
